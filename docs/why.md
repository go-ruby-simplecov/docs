...
`go-ruby-simplecov/simplecov` reimplements Ruby's `simplecov` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-simplecov/simplecov` directly, with no Ruby runtime;
...
Because `simplecov` is pure Go with cgo disabled, it also compiles to **WebAssembly**
